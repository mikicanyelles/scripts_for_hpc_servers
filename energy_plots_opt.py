        ...
        try :
            ...
            if len(files) == 1:
                ...
                for f in range(len(files)):
                    print(f+1, '. ', files[f])
                try :
                    selected = int(input('Select which file do you want (1-%s)' % len(files)))
                    if selected in [x for x in range(1,len(files)+1)]:
                        print(files[selected-1], ' will be used')
                        file_name = files[selected-1]
                except ValueError:
                    print('Answer a number. Bye.')
                    exit(0)
            elif len(files) == 0:
                print('No .out file have been detected. If the extension is different from .out, specify the file name as an argument')
...
def main():
    ...
    print('The number of calculated points is: ', len(energy[0]))
    try :
        if argv[2] == 'csv':
            write_csv(energy)
